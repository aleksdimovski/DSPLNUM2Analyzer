...
let isLeq t1 t2 =
  ...
  (**)
    ...
    let rec propagateCons env_feats feats env_vars vars t cs = 
		(match t with
      	| Leaf p ->
			...
			Leaf (P.meet p p')
      	| Node ((c1,nc1),l1,r1) ->
       		let l = propagateCons env_feats feats env_vars vars l1 (c1::cs) in
			let r = propagateCons env_feats feats env_vars vars r1 (nc1::cs) in
			Node ((c1,nc1),l,r)
  ...
  (**)
    ...
    in { domain = domain;
      tree = propagateCons env_feats feats env_vars vars (aux t.tree []) [];
      env_vars = env_vars;
	  env_feats = env_feats;
      vars = vars;
	  feats = feats }	  	
 (**)
  ...
  (**)
	...
	( match ccs1 with 
	| [] -> l1 (*Leaf (P.bot env_vars vars) *)
	| [(c,nc)] -> Node ((c,nc),l1,tree2_r ccs2 (nc::cs) r1) 
	| (c,nc)::xs -> Node ((c,nc),tree2_l xs (c::cs) l1,tree2_r ccs2 (nc::cs) r1) ) in
    ...
    let rec aux t cs = match t with
      | Leaf p ->
        let p' = match domain with 
          ...
          | Some domain -> 
            P.inner env_feats feats (cs@(P.constraints domain)) in
		let p1 = P.fwdAssign p (l,ee) in
		if (!hasNode) then (
        	if (P.isBot p') then (Leaf (P.bot env_vars vars), [Lincons1.make_unsat env_vars])
        	else (Leaf p1, P.constraints p1) 
		) else (
			let p1' = P.project p1 env_feats feats in 
			let dcs = ref [] in let ccs1 = ref [] in
			dcs:=(match domain with | None -> [] | Some domain -> P.constraints domain); 	
			List.iter (fun c -> if ((not (List.mem c !dcs)) && (not (C.isBot c))) then if (not (P.isLeq p' (P.meet p' (P.inner env_feats feats [c])))) then (let nc = C.negate c in ccs1:=(c,nc)::!ccs1)) (P.constraints p1');
			List.iter (fun (c,nc) -> Format.fprintf !fmt "\n ccs1: c: %a : nc: %a \n" (C.print feats) c (C.print feats) nc) !ccs1;
			(*(tree2_l !ccs1 [] (Leaf p1), P.constraints p1) *)
			(Leaf p1, P.constraints p1')
		)
      | Node ((c1,nc1),l1,r1) ->
	    hasNode := true; 
	  	Format.fprintf !fmt "\n Enter aux Node: c1 %a :\n l1 %a :\n r1 %a :\n" (C.print feats) c1 (print_tree env_feats feats) l1 (print_tree env_feats feats) r1;
        let (lt,pl1) = aux l1 (c1::cs) in
		let (rt,pr1) = aux r1 (nc1::cs) in 			  
		...
		List.iter (fun c -> let nc = C.negate c in ccs1:=(c,nc)::!ccs1) pl1;
		if ((List.length pr1)>0) then 
		List.iter (fun nc -> let c = C.negate nc in if (List.mem (c,nc) !ccs1) then (ccs_common:=(c,nc)::!ccs_common; ccs1:=List.filter (fun (c1,nc1) -> not (C.isEq c c1 && C.isEq nc nc1) ) !ccs1) else ccs2:=(c,nc)::!ccs2) pr1; 		
		...
		if (b1 && b2) then ( 
			...
			if (P.isBot p1p2) then (
				...
				(*tree_l ccs1 cs l1 r1 jj ccsj ccs2
				
				if ((List.length !ccs1) + (List.length !ccs_common) = 1) then (
					let (cc1,cnc1) = if ((List.length !ccs_common)=0) then List.hd !ccs1 else List.hd !ccs_common in 
					let (l,pl) = aux l1 (cc1::cs) in
					if ((List.length !ccs2)=0) then (
						let (r,pr) = aux r1 (cnc1::cs) in Format.fprintf !fmt "\n case 1 \n"; 
						if (P.isLeq p2 p1) then (Node ((cc1,cnc1),join_t env_vars vars env_feats feats l r [] domain,Leaf (P.bot env_vars vars)),pl1@pr1) else (Node ((cc1,cnc1),l,r),pl1@pr1) )
					else (
						let (cc2,cnc2) = List.hd !ccs2 in
						let rl = Leaf (P.bot env_vars vars) in 	
						let (rr,prr) = aux r1 (cnc1::cnc2::cs) in Format.fprintf !fmt "\n case 2 \n"; 	
						(Node ((cc1,cnc1),l,Node ((cc2,cnc2),rl,rr)),pl1@pr1) ) )
				else (
					let (cc1,cnc1) = if ((List.length !ccs_common)=0) then List.hd !ccs1 else List.hd !ccs_common in 
					let (cc2,cnc2) = if ((List.length !ccs_common)=0) then List.hd (List.tl !ccs1) else List.hd !ccs1 in 
					let (ll,pll) = aux l1 (cc1::cc2::cs) in
					if ((List.length !ccs2)=0) then (
							let (lr,plr) = aux r1 (cnc1::cc2::cs) in 
							let (rr,prr) = aux r1 (cnc1::cnc2::cs) in Format.fprintf !fmt "\n case 3 \n"; 
							Format.fprintf !fmt "\n ll: %a : lr: %a : rr: %a \n" (print_tree env_feats feats) ll (print_tree env_feats feats) lr (print_tree env_feats feats) rr;
							let jj = join_t env_vars vars env_feats feats (join_t env_vars vars env_feats feats ll rr [] domain) lr [] domain in 							
							Format.fprintf !fmt "\n jj: %a \n" (print_tree env_feats feats) jj;
							if (P.isLeq p2 p1) 
							then (Node ((cc1,cnc1),Node ((cc2,cnc2),jj,Leaf (P.bot env_vars vars)),Leaf (P.bot env_vars vars)),pl1@pr1)
							else (Node ((cc1,cnc1),Node ((cc2,cnc2),ll,lr),rr),pl1@pr1) )
					else (
							let (ccc2,ccnc2) = List.hd !ccs2 in
							let rl = Leaf (P.bot env_vars vars) in 	
							let (rr,prr) = aux r1 (cnc1::ccnc2::cs) in Format.fprintf !fmt "\n case 4 \n";  	
							(Node ((cc1,cnc1),Node ((cc2,cnc2),ll,Leaf (P.bot env_vars vars)),Node ((ccc2,ccnc2),rl,rr)),pl1@pr1)
					)						
				 )  *)
			...
			(*if ((List.length !ccs1) + (List.length !ccs_common) = 1) then (
				...
				if ((List.length !ccs2)=0) then (
					...
					let (rr,prr) = aux r1 (cnc1::cnc2::cs) in Format.fprintf !fmt "\n case 2 \n"; 	
					(Node ((cc1,cnc1),l,Node ((cc2,cnc2),rl,rr)),pl1@pr1) ) )  
				else if ((List.length !ccs1) + (List.length !ccs_common) = 2) then (
					let (cc1,cnc1) = if ((List.length !ccs_common)=0) then List.hd !ccs1 else List.hd !ccs_common in 
					let (cc2,cnc2) = if ((List.length !ccs_common)=0) then List.hd (List.tl !ccs1) else List.hd !ccs1 in 
					let (ll,pll) = aux l1 (cc1::cc2::cs) in
					if ((List.length !ccs2)=0) then (
				...
				else if ((List.length !ccs1) + (List.length !ccs_common) = 3) then (
					let (cc1,cnc1) = if ((List.length !ccs1)>0) then List.hd !ccs1 else List.hd !ccs_common in 
					let (cc2,cnc2) = if ((List.length !ccs_common)=0) then List.hd (List.tl !ccs1) else List.hd !ccs1 in 
					let (ll,pll) = aux l1 (cc1::cc2::cs) in
					if ((List.length !ccs2)=0) then (
							let (lr,plr) = aux r1 (cnc1::cc2::cs) in 
	...
	let rec aux t e cs =
        (match t with
         | Leaf p -> 
			 		...
			 		let super_env = env_vars (*Environment.lce env_vars env_feats*) in 
			 		let cnstr1 = List.map (fun c -> Lincons1.extend_environment c super_env) (P.constraints b) in (*EXTEND EVIRONMENT HERE*) 
			 		let cnstr2 = P.constraints p (*List.map (fun c -> Lincons1.extend_environment c super_env) (P.constraints p)*) in (*EXTEND EVIRONMENT HERE*)
			 		let super_cons = cnstr1@cnstr2 in 					
			 		let super_vars = vars(*@feats*) in 
			 		let super_p = P.inner super_env super_vars super_cons in 
		 			...
		 			let p' = super_p' (*P.project super_p' env_vars vars*) in
					let p'' = P.project super_p' env_feats feats in 
					Format.fprintf !fmt "\n General filter p' leaf: %a : p'' node: %a \n" P.print p' P.print p''; 
					if (P.isLeq b p'') then (Leaf p') (*Format.fprintf !fmt "\n p'' is redundant \n" *)
					else ( 	toSort := true; 
							let dcs = match domain with | None -> [] | Some domain -> P.constraints domain in 
							let cs = ref [] in 
							List.iter (fun c -> if ((not (List.mem c dcs)) && (not (C.isBot c))) then (let nc = C.negate c in cs:=(c,nc)::!cs)) (P.constraints p'');
							(*let ft1 = if (!cs=[]) then (Leaf p') else Node (List.hd !cs,Leaf p',Leaf (P.bot env_vars vars)) in Format.fprintf !fmt "\n p'' is not redundant \n"; *)
							let ft2 = tree2_l !cs [] (Leaf p') in 
