...
let rec aExp_toFeats e features isLinear =
  match e with
  | A_RANDOM -> []
  | A_URANDOM -> []
  | A_var x -> let ret = ref [] in List.iter (fun ft -> if (x.varId==ft.varId) then ((*Format.fprintf Format.std_formatter "CHECK ";*) ret:=[(x,isLinear)])) features; !ret 
  | A_const i -> []
  | A_interval (i1,i2) -> []
  | A_aunary (o,(e,_)) ->
    let b = aExp_toFeats e features true in
    (match o with
     | A_UMINUS -> b)
  | A_abinary (o,(e1,_),(e2,_)) ->
	(match o with
     | A_PLUS | A_MINUS -> 
	 	    let e1 = aExp_toFeats e1 features true in
    		let e2 = aExp_toFeats e2 features true in
    		e1@e2
	 | A_MULTIPLY | A_DIVIDE | A_MOD -> 
	 	    let e1 = aExp_toFeats e1 features false in
    		let e2 = aExp_toFeats e2 features false in
    		e1@e2 ) 
  | _ -> []
...
let rec bExp_toFeats e features isLinear =
  match e with
  | A_TRUE | A_MAYBE | A_FALSE -> []
  | A_bvar x -> let ret = ref [] in List.iter (fun ft -> if (x.varId==ft.varId) then (ret:=[(x,isLinear)])) features; !ret 
  | A_bunary (o,(e,_)) ->
    let b = bExp_toFeats e features true in
    (match o with
     | A_NOT -> b)
  | A_bbinary (o,(e1,_),(e2,_)) ->
 	    let e1 = bExp_toFeats e1 features true in
   		let e2 = bExp_toFeats e2 features true in
   		e1@e2
